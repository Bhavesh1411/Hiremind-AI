...
def _wait_for_evaluations_and_advance():
    ...
    if session_id:
        ...
        # ── 5. Generate Final AI Analysis Report
        report = generate_combined_report(
            stage1_answers  = s1_answers,
            stage2_results  = [r for r in v_results if r],
            voice_questions = st.session_state.get("voice_questions", []),
            api_key         = os.getenv("GEMINI_API_KEY", "")
        )
        ai_report_json = json.dumps(report.get("ai_analysis", {}))

        # ── 6. Finalise in DB
        finalize_interview(session_id, final_score=interview_score_100, ats_score=ats_score, ai_report=ai_report_json)
...
def render_combined_report():
    ...
    with st.container():
        ...
        report_key = f"final_report_{session_id}"
        if report_key not in st.session_state:
            # First, check if we already have a stored AI report in the database session object
            stored_ai_json = session.get("ai_report", "")
            if stored_ai_json:
                try:
                    stored_ai = json.loads(stored_ai_json)
                    report = {
                        "stage1": {
                            "answers": s1_answers,
                            "total": sum(a.get("score", 0) for a in s1_answers),
                            "max": len(s1_answers) * 10,
                            "percentage": round((sum(a.get("score", 0) for a in s1_answers) / (len(s1_answers)*10) * 100) if s1_answers else 0, 1),
                        },
                        "stage2": {
                            "results": s2_results,
                            "avg_score": round(sum(r.get("score", 0) for r in s2_results)/len(s2_results), 1) if s2_results else 0,
                            "percentage": round((sum(r.get("score", 0) for r in s2_results)/(len(s2_results)*10)*100) if s2_results else 0, 1),
                        },
                        "combined": {
                            "percentage": session.get("final_score", 0),
                        },
                        "ai_analysis": stored_ai
                    }
                    # Recalculate combined weighted if necessary
                    s1_p = report["stage1"]["percentage"]
                    s2_p = report["stage2"]["percentage"]
                    report["combined"]["percentage"] = round(s1_p * 0.6 + s2_p * 0.4, 1)
                    
                    # Add verdict logic
                    cp = report["combined"]["percentage"]
                    if cp >= 75: report["combined"].update({"verdict": "Strong Hire", "verdict_color": "#10b981", "verdict_icon": "🟢"})
                    elif cp >= 55: report["combined"].update({"verdict": "Hire", "verdict_color": "#f59e0b", "verdict_icon": "🟡"})
                    else: report["combined"].update({"verdict": "No Hire", "verdict_color": "#ef4444", "verdict_icon": "🔴"})
        ...
        <div class="voice-header">
            ...
            <p style="margin:0.4rem 0 0; color:#475569;">
                Candidate: <strong style="color:#1e293b;">{candidate}</strong>
        ...
        st.markdown(f"""
        <div class="verdict-banner" style="background:{vc}15;border:2px solid {vc};">
